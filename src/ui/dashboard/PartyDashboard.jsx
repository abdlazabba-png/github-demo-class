    ...
    <div className="dashboard">
      <div className="dashboard-header">
        ...
        <p className="hint">
          ...
          this client — the dashboard's data layer has no function capable of returning another
          client's data (CLAUDE.md: isolation enforced at the access-control layer, not the UI).
        </p>
      </div>
      ...
      {activeTab === 'coverage' && (
        <CoverageView server={server} partyClientId={partyClientId} refreshToken={refreshToken} />
      )}
      {activeTab === 'evidence' && (
        <EvidenceView server={server} partyClientId={partyClientId} refreshToken={refreshToken} />
      )}
      {activeTab === 'discrepancies' && (
        <DiscrepancyQueue server={server} partyClientId={partyClientId} refreshToken={refreshToken} />
      )}
      {activeTab === 'audit' && (
        <AuditLogView server={server} partyClientId={partyClientId} refreshToken={refreshToken} />
      )}
    </div>
  );
}
